...
#define MQTT_HOST "183.230.40.96"		//ip地址
#define MQTT_PORT 1883					//端口号
#define MQTT_USER "436577"				//用户名
#define MQTT_PASS "version=2018-10-31&res=products%2F436577%2Fdevices%2Ficm&et=1920526869&method=md5&sign=tkloNhVAm7gjAhlBWThuLQ%3D%3D"			//密码
#define MQTT_CLIENT_ID "icm"		//客户端标识
...
void iot_mqtt_init(Cloud_MQTT_t *piot_mqtt) 
{
    ...
    sprintf(piot_mqtt->sub_topic, "%s%scmd/#", gateway.model, gateway.company);	//将初始化好的订阅主题填到数组中
    printf("subscribe:%s\n", piot_mqtt->sub_topic);

    sprintf(piot_mqtt->pub_topic, "%s%sdp/post/json", gateway.model, gateway.company);	//将初始化好的发布主题填到数组中
    printf("pub:%s\n", piot_mqtt->pub_topic);
